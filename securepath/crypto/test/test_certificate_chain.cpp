...
TEST_CASE("certificate_chain", "[certificate_chain]") {
	...
	CHECK(chain->is_authentic(pki.keys));
	...
	CHECK(partial_chain->is_authentic(pki.keys));
	...
	CHECK(empty.is_valid());
	CHECK(empty.is_authentic(pki.keys));
}
...
TEST_CASE("certificate_chain serialisation", "[certificate_chain][serialisation]") {
	...
	CHECK(restored.is_authentic(pki.keys));
...
TEST_CASE("certificate_chain uses the process-wide root key by default", "[certificate_chain][root_public_key]") {
	...
	CHECK_THROWS(root_public_key());
	{
		...
		CHECK(pki.ca_chain.is_authentic(keys));
		private_key server = generate_private_key();
		auto chain = pki.chain_for_server_key(server, "host.example.org");
		CHECK(chain.is_authentic(keys));
		CHECK(chain.is_authentic(keys, key_cert_restriction().hostname("host.example.org")));
		CHECK(chain.is_authentic(keys, key_cert_restriction().hostname("sub.host.example.org")));
		CHECK(!chain.is_authentic(keys, key_cert_restriction().hostname("other.example.org")));
		...
		CHECK(!chain.is_authentic(other_root));
	}
...
TEST_CASE("certificate_chain invalid chains are rejected", "[certificate_chain][security]") {
	...
	SECTION("ca level violation: a ca 0 key certifies another key") {
		...
		CHECK(!forced.is_authentic(pki.keys));
	}
...
TEST_CASE("certificate_chain hostname restrictions", "[certificate_chain][security]") {
	...
	CHECK(chain->is_authentic(pki.keys, key_cert_restriction().hostname("a.example.org")));
	CHECK(chain->is_authentic(pki.keys, key_cert_restriction().hostname("deep.a.example.org")));
	CHECK(chain->is_authentic(pki.keys, key_cert_restriction().hostname("A.EXAMPLE.ORG")));
	CHECK(!chain->is_authentic(pki.keys, key_cert_restriction().hostname("b.example.org")));
	CHECK(!chain->is_authentic(pki.keys, key_cert_restriction().hostname("example.org")));
	CHECK(!chain->is_authentic(pki.keys, key_cert_restriction().hostname("xa.example.org")));
	CHECK(!chain->is_authentic(pki.keys, key_cert_restriction()));
	...
	CHECK(!forced.is_authentic(wide.keys));
}
